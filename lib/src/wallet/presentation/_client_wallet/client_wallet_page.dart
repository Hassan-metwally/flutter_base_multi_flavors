...
import '../charge_wallet/charage_wallet_bottom_sheet.dart';
import '../widgets/transaction_card.dart';
import '../widgets/transaction_loading_card.dart';
import 'client_wallet_cubit.dart';
import 'utils/client_wallet_subscription.dart';
...
class _ClientWalletPageBodyState extends State<_ClientWalletPageBody> {
  final _clientWalletSubscriptionObj = CompositeSubscription();
  final ScrollController _scrollController = ScrollController();
  late final ClientWalletCubit _cubit;

  void _clientWalletSubsriptionListener() {
    _clientWalletSubscriptionObj.add(
      UserWalletSubscription.stream().listen((params) {
        _cubit.getBalance();
  ...
  void initState() {
    ...
    _clientWalletSubsriptionListener();
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent) {
        context.read<ClientWalletCubit>().getMoreWalletHistory();
      }
    });
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () => ChargeWalletBottomSheet.show(context: context),
            child: Container(
              ...
              child: Row(
                children: [
                  AppSvgIcon(path: "", width: 12, height: 12, color: AppColors.primary),
                  const SizedBox(width: 4),
                  Text(appLocalizer.walletTopUp, style: TextStyles.regular12.copyWith(color: AppColors.primary)),
                ],
              ),
            ),
          ),
        ...
        return Container(
          ...
          child: SafeArea(
            ...
            child: AppButton(
              ...
              onPressed: () => _WithdrawBalanceBottomSheet.show(context: context, userWalletCubit: ctx.read()),
            ),
          ),
        );
      },
...
class _WithdrawBalanceBottomSheet extends StatefulWidget {
  final ClientWalletCubit userWalletCubit;
  const _WithdrawBalanceBottomSheet({required this.userWalletCubit});

  static Future<bool?> show({required BuildContext context, required ClientWalletCubit userWalletCubit}) async =>
      await showAppModalBottomSheet(
        context: context,
        child: _WithdrawBalanceBottomSheet(userWalletCubit: userWalletCubit),
      );
...
class _WithdrawBalanceBottomSheetState extends State<_WithdrawBalanceBottomSheet> {
                      ...
                      child: BlocConsumer<ClientWalletCubit, ClientWalletState>(
                        listener: (context, state) {
                          if (state.withDrawState.isSuccess) {
                            ...
                            widget.userWalletCubit.getBalance();
                        ...
                        builder: (ctx, state) {
                          return AppButton(
                            ...
                            onPressed: () => onWithdrawl(context, widget.userWalletCubit),
                          );
                        },
                      ),
                    ),
  ...
  void onWithdrawl(BuildContext context, ClientWalletCubit userWalletCubit) async {
    ...
    userWalletCubit.withdrawBalance(amount: amount, bank: bank, accountName: accountName, accountNumber: accountNumber, iban: iban);
  }
}
