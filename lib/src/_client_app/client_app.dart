...
import '../authentication/presentation/login/client_login_page.dart';
import '../authentication/presentation/login/login_cubit.dart';
import '../common/presentation/splash/splash_page.dart';
import 'main_page/client_main_page.dart';
import 'onboarding/client_onboarding_page.dart';
                ...
                builder: (context, themeValue, child) {
                  return MaterialApp(
                    ...
                    onGenerateInitialRoutes: (String initialRoute) {
                      return [AppRoutesGenerator.generateRoute(const RouteSettings(name: AppRoutes.clientHome))];
                    },
                  );
                },
              ),
...
class ClientBuilderScreen extends StatelessWidget {
  const ClientBuilderScreen({super.key});
    ...
    return BlocBuilder<AppAuthenticationBloc, AppAuthenticationState>(
      builder: (context, state) {
        ...
        if (state is AuthUninitialized) {
          ...
          root = const ClientOnboardingPage();
        } else if (state is AuthLogInPageState || state is AuthLogOutState) {
          root = BlocProvider(create: (context) => LoginCubit(), child: const ClientLoginPage());
        } else {
