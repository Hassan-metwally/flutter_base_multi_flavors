...
class _ClientOnboardingPageState extends State<ClientOnboardingPage> {
  int _currentPage = 0;
  final List<Widget> _imagesWidgets = [AppImages.userOnboarding1, AppImages.userOnboarding2, AppImages.userOnboarding3]
      .map(
        (item) => Center(
          child: AppImage(path: item, cacheImage: true, fit: BoxFit.fill),
        ),
      )
      .toList();
  List<Widget> get _titleWidgets => [
    appLocalizer.userOnBoardingTitle1,
    appLocalizer.userOnBoardingTitle2,
    appLocalizer.userOnBoardingTitle3,
  ].map((item) => _TitleWidget(text: item)).toList();
  List<Widget> get _subTitleWidgets => [
    appLocalizer.userOnBoardingSubTitle1,
    appLocalizer.userOnBoardingSubTitle2,
    appLocalizer.userOnBoardingSubTitle3,
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: AnimatedSwitcher(
                  ...
                  child: Center(
                    child: SizedBox(height: 480, key: ValueKey(_currentPage), child: _imagesWidgets[_currentPage]),
                  ),
                ),
              ),
            ),
              ...
              children: [
                Expanded(
                  child: AnimatedSwitcher(
                    ...
                    reverseDuration: Durations.medium4,
                    child: SizedBox(key: ValueKey(_currentPage), child: _titleWidgets[_currentPage]),
                  ),
                ),
              ],
            ),
              ...
              children: [
                Expanded(
                  child: AnimatedSwitcher(
                    duration: Durations.medium4,
                    reverseDuration: Durations.medium4,
                    child: SizedBox(key: ValueKey(_currentPage), child: _subTitleWidgets[_currentPage]),
                  ),
                ),
              ],
            ),
...
class _TitleWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      ...
      style: TextStyles.medium18.copyWith(color: AppColors.primary600),
    );
  }
}
...
class _SubTitleWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      text,
      textAlign: TextAlign.center,
      style: TextStyles.regular14.copyWith(color: AppColors.black700),
      maxLines: 3,
    );
  }
}
...
class RPSCustomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint0Stroke = Paint()
      ...
      ..strokeWidth = 2;
    paint0Stroke.color = AppColors.primary500;
